...
- **目的**: 全員への挨拶とメニュー提案
- **動作**: 
  1. データベースから全 `Person` とそれに関連する `FavoriteFood` を取得します。
  2. 取得した好物の名称（「和食」「イタリアン」など）を [TheMealDB API](https://www.themealdb.com/) が理解できるキーワード（`Japanese`, `Italian` など）にマッピングします。
  3. API 経由で、実際の料理名（例: 「Sushi」, 「Lasagna」）を Web から動的に取得します。
  4. 一人ひとりに向けたパーソナライズされたメッセージを生成し、実際の料理名を添えて提案します。
- **外部連携**: 
  - API: TheMealDB (Free Tier)
  - キーワードマッピング: 和食, ファストフード, 中華, イタリアン, フレンチ, インド, タイ, メキシカン, 海鮮, 牛肉, 鶏肉, パスタ, デザートに対応しています。マッピングにない場合はデフォルトの提案を行います。
